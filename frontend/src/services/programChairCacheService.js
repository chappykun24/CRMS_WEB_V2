// Program Chair-specific caching service for enhanced performance
class ProgramChairCacheService {
  constructor() {
    this.cache = {
      analytics: new Map(),
      classes: new Map(),
  ...
  get(cacheType, key, customTTL = null) {
    ...
    if (isExpired) {
      cache.delete(key);
      console.log(`⏰ [PROGRAM CHAIR CACHE] Expired ${cacheType}:${key}`);
      return null;
    }
    
    console.log(`📦 [PROGRAM CHAIR CACHE] Hit ${cacheType}:${key}`);
    return cached.data;
  }
  ...
  set(cacheType, key, data, customTTL = null) {
    ...
    console.log(`💾 [PROGRAM CHAIR CACHE] Stored ${cacheType}:${key}`);
    return true;
  }
  ...
  clear(cacheType, key = null) {
    ...
    if (key) {
      cache.delete(key);
      console.log(`🧹 [PROGRAM CHAIR CACHE] Cleared ${cacheType}:${key}`);
    } else {
      cache.clear();
      this.cache.lastUpdated.delete(cacheType);
      console.log(`🧹 [PROGRAM CHAIR CACHE] Cleared all ${cacheType}`);
    }
    return true;
  }
  ...
  clearAll() {
    ...
    console.log('🧹 [PROGRAM CHAIR CACHE] Cleared all caches');
  }
  ...
  cleanCache(cacheType) {
    ...
    if (cleaned > 0) {
      console.log(`🧹 [PROGRAM CHAIR CACHE] Cleaned ${cleaned} expired entries from ${cacheType}`);
    }
  }
  ...
  async getWithRefresh(cacheType, key, fetchFunction, customTTL = null) {
    ...
    try {
      ...
      console.error(`❌ [PROGRAM CHAIR CACHE] Failed to fetch ${cacheType}:${key}`, error);
      throw error;
    }
  }
...
const programChairCacheService = new ProgramChairCacheService();

export default programChairCacheService;
