...
const Syllabus = () => {
 ...
 // Reset selected syllabus when class changes
  ...
  const handleDeleteSyllabus = async (syllabusId) => {
    ...
    // Extract syllabus_id directly - just get the numeric ID
    let targetSyllabusId = targetSyllabus.syllabus_id || targetSyllabus.id || syllabusId
    
    // Convert to number if it's a string
    if (typeof targetSyllabusId === 'string') {
      targetSyllabusId = parseInt(targetSyllabusId, 10)
    }

    // Validate the ID
    if (!targetSyllabusId || typeof targetSyllabusId !== 'number' || isNaN(targetSyllabusId) || targetSyllabusId <= 0) {
      const title = targetSyllabus?.title || 'Unknown'
      console.error('❌ [DELETE] Invalid syllabus ID for deletion:', {
        syllabus_id: targetSyllabus.syllabus_id,
        id: targetSyllabus.id,
        syllabusId_param: syllabusId,
        extracted_id: targetSyllabusId,
        title: title
      })
    ...
    // Check if it's a draft (only drafts can be deleted)
    ...
    // Confirm deletion
    const title = targetSyllabus?.title || 'this syllabus'
          ...
          <div className="bg-white rounded-lg max-w-7xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                {/* Draft Section - Show Edit, Submit, Delete, and Share buttons for draft syllabi 
                  ...
                  <div className="pt-4 border-t border-gray-300">
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        className="flex-1 px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {deletingSyllabus ? (
                          <>
                ...
                {/* Share to Class Section - Show for non-approved syllabi that are not drafts */}
                {!(viewingSyllabus.status === 'draft' || 
                  (viewingSyllabus.review_status === 'pending' && 
                   viewingSyllabus.approval_status === 'pending')) &&
                 !(viewingSyllabus.approval_status === 'approved' && viewingSyllabus.review_status === 'approved') && (
