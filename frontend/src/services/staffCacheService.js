// Staff-specific caching service for enhanced performance
class StaffCacheService {
  ...
  get(cacheType, key, customTTL = null) {
    ...
    if (isExpired) {
      cache.delete(key);
      console.log(`⏰ [STAFF CACHE] Expired ${cacheType}:${key}`);
      return null;
    }
    
    console.log(`📦 [STAFF CACHE] Hit ${cacheType}:${key}`);
    return cached.data;
  }
  ...
  set(cacheType, key, data, customTTL = null) {
    ...
    console.log(`💾 [STAFF CACHE] Stored ${cacheType}:${key}`);
    return true;
  }
  ...
  clear(cacheType, key = null) {
    ...
    if (key) {
      cache.delete(key);
      console.log(`🧹 [STAFF CACHE] Cleared ${cacheType}:${key}`);
    } else {
      cache.clear();
      this.cache.lastUpdated.delete(cacheType);
      console.log(`🧹 [STAFF CACHE] Cleared all ${cacheType}`);
    }
    return true;
  }
  ...
  clearAll() {
    ...
    console.log('🧹 [STAFF CACHE] Cleared all caches');
  }
  ...
  cleanCache(cacheType) {
    ...
    if (cleaned > 0) {
      console.log(`🧹 [STAFF CACHE] Cleaned ${cleaned} expired entries from ${cacheType}`);
    }
  }
  ...
  async getWithRefresh(cacheType, key, fetchFunction, customTTL = null) {
    ...
    try {
      ...
      console.error(`❌ [STAFF CACHE] Failed to fetch ${cacheType}:${key}`, error);
      throw error;
    }
  }
