...
// Cache helpers
const getCachedData = createCacheGetter(facultyCacheService);
const setCachedData = createCacheSetter(facultyCacheService);
...
const Analytics = () => {
  ...
  const [facultyClasses, setFacultyClasses] = useState([]); // Faculty's classes for selected term
  ...
  const fetchSchoolTerms = useCallback(async () => {
    console.log('🔍 [FACULTY ANALYTICS] fetchSchoolTerms starting');
    
    // Check sessionStorage first for instant display
    const sessionCacheKey = 'faculty_school_terms_session';
    const sessionCached = safeGetItem(sessionCacheKey);
    
    // Check enhanced cache
    const cacheKey = 'faculty_school_terms';
    const cachedData = getCachedData('terms', cacheKey, 30 * 60 * 1000); // 30 minute cache
    
    if (sessionCached) {
      console.log('📦 [FACULTY ANALYTICS] Using session cached school terms');
      ...
      console.log('📦 [FACULTY ANALYTICS] Using enhanced cached school terms');
      // Sort terms to prioritize latest term
    ...
    try {
      console.log('🔄 [FACULTY ANALYTICS] Fetching fresh school terms...');
      ...
      console.log(`✅ [FACULTY ANALYTICS] Received ${Array.isArray(terms) ? terms.length : 0} school terms`);
      ...
      if (error.name === 'AbortError') {
        console.log('🚫 [FACULTY ANALYTICS] School terms request was aborted');
        return;
      }
      console.error('❌ [FACULTY ANALYTICS] Error fetching school terms:', error);
      ...
      if (!hasCachedTerms && setErrorRef.current) {
        try {
          ...
          console.warn('⚠️ [FACULTY ANALYTICS] Could not set error state (component may have unmounted)');
        }
      }
    }
  }, []);

  // Fetch faculty classes for selected term
  const fetchFacultyClasses = useCallback(async (termId) => {
    ...
    try {
      ...
      console.log(`✅ [FACULTY ANALYTICS] Found ${termClasses.length} classes for term ${termId}`);
      return termClasses;
    } catch (error) {
      if (error.name !== 'AbortError') {
  ...
  const loadStudentPhoto = useCallback(async (studentId) => {
    ...
    try {
      ...
      console.error('❌ [FACULTY ANALYTICS] Error loading student photo:', error);
      // Cache null to prevent retrying failed loads
      studentPhotoCache.current.set(studentId, null);
      setStudentPhoto(null);
    } finally {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          console.log('✅ [FACULTY ANALYTICS] Loaded student enrollments:', result.data.length);
        }
      }
    } catch (error) {
      console.error('❌ [FACULTY ANALYTICS] Error loading student enrollments:', error);
  ...
  const fetchClassAnalytics = useCallback(async (studentId, sectionCourseId) => {
    ...
    try {
      ...
      console.log('🔄 [FACULTY ANALYTICS] Fetching class analytics:', url);
      ...
      if (response.ok) {
        ...
        if (result.success && result.data && result.data.length > 0) {
          ...
          console.log('✅ [FACULTY ANALYTICS] Loaded class-specific analytics:', {
            student_id: studentId,
            section_course_id: sectionCourseId,
            data: classData
          });
          ...
          console.warn('⚠️ [FACULTY ANALYTICS] No class-specific data found in response:', result);
          setClassFilteredData(null);
        }
      } else {
        ...
        try {
          ...
          try {
            ...
            console.error('❌ [FACULTY ANALYTICS] Non-JSON error response:', textResponse);
          }
        } catch (textError) {
          // Failed to read response body
          console.error('❌ [FACULTY ANALYTICS] Failed to read error response:', textError);
        }
        
        console.error('❌ [FACULTY ANALYTICS] Failed to fetch class analytics:', {
        ...
        if (response.status === 500) {
          console.error('❌ [FACULTY ANALYTICS] Backend 500 error details:', {
            error: errorMessage,
            errorCode: errorCode,
            errorDetails: errorDetails,
            queryParams: {
          ...
          console.warn('⚠️ [FACULTY ANALYTICS] Backend error (500) - using overall data as fallback');
      ...
      console.error('❌ [FACULTY ANALYTICS] Error loading class analytics:', {
        error: error.message,
        student_id: studentId,
        section_course_id: sectionCourseId
      });
  ...
  const handleFetch = useCallback(async (forceRefresh = false) => {
    ...
    if (!facultyId || !selectedTermId) {
      console.warn('⚠️ [FACULTY ANALYTICS] Missing facultyId or selectedTermId');
      return;
    }
    ...
    if (forceRefresh) {
      console.log('🔄 [FACULTY ANALYTICS] Force refresh - clearing caches...');
      const sessionCacheKey = `faculty_analytics_${filterKey}_session`;
      const cacheKey = `faculty_analytics_${filterKey}`;
      
      // Clear sessionStorage cache
      try {
        sessionStorage.removeItem(sessionCacheKey);
        console.log('✅ [FACULTY ANALYTICS] Cleared sessionStorage cache');
      } catch (e) {
        console.warn('⚠️ [FACULTY ANALYTICS] Could not clear sessionStorage:', e);
      }
      
      // Clear enhanced cache
      try {
        facultyCacheService.clear('analytics', cacheKey);
        console.log('✅ [FACULTY ANALYTICS] Cleared enhanced cache');
      } catch (e) {
        console.warn('⚠️ [FACULTY ANALYTICS] Could not clear enhanced cache:', e);
      }
    ...
    if (sessionCached && sessionCached.success) {
      console.log('📦 [FACULTY ANALYTICS] Using session cached analytics data');
    ...
    if (cachedData && cachedData.success && !sessionCached) {
      console.log('📦 [FACULTY ANALYTICS] Using enhanced cached analytics data');
    ...
    try {
      ...
      const classes = await fetchFacultyClasses(parseInt(selectedTermId));
      
      if (!classes || classes.length === 0) {
        console.warn('⚠️ [FACULTY ANALYTICS] No classes found for selected term');
        setData([]);
      ...
      setProgress(30);
      const analyticsPromises = classes.map(async (classItem, index) => {
        try {
          const params = new URLSearchParams();
          params.append('section_course_id', classItem.section_course_id.toString());
          // Add Standard filter if selected
          ...
          if (result.success && Array.isArray(result.data)) {
            ...
            return {
              ...
              clustering: result.clustering || { enabled: false }
            };
          }
          return null;
        } catch (error) {
      ...
      // Wait for all class analytics to complete
      setProgress(60);
      ...
      // Determine clustering meta from results (use first non-null clustering result)
      const clusteringMeta = clusteringResults.find(c => c && c.enabled) || { enabled: false };
      if (clusteringResults.length > 0) {
        // Use the first clustering result's metadata
        const firstClustering = clusteringResults[0];
        if (firstClustering) {
          clusteringMeta.silhouetteScore = firstClustering.silhouetteScore;
          clusteringMeta.backendPlatform = firstClustering.backendPlatform;
          clusteringMeta.apiPlatform = firstClustering.apiPlatform;
        }
      }
      ...
      if (studentsWithClusters.length === 0 && aggregatedStudents.length > 0) {
        ...
        if (setErrorRef.current) {
          try {
            ...
            console.warn('⚠️ [FACULTY ANALYTICS] Could not set error state');
          }
        }
      }

      // Create response object matching dean's format
      const aggregatedResponse = {
        success: true,
        data: aggregatedStudents,
        clustering: clusteringMeta
      };

      setData(aggregatedStudents);
      setClusterMeta(clusteringMeta);
      ...
      // Store in sessionStorage for instant next load
      const sessionCacheKey = `faculty_analytics_${filterKey}_session`;
      safeSetItem(sessionCacheKey, aggregatedResponse);
      
      // Store full data in enhanced cache
      const cacheKey = `faculty_analytics_${filterKey}`;
      setCachedData('analytics', cacheKey, aggregatedResponse);
      ...
      try {
        trackEvent('faculty_analytics_loaded', {
          success: true,
          count: aggregatedStudents.length,
          clustering_enabled: Boolean(clusteringMeta?.enabled),
          platform: clusteringMeta?.platform || 'Unknown',
        });
      } catch {}
    } catch (err) {
      if (progressIntervalRef.current) {
      ...
      setChartsLoaded(false);
      try {
        trackEvent('faculty_analytics_error', { message: String(err?.message || err) });
      } catch {}
    }
  }, [facultyId, selectedTermId, selectedYearLevel, selectedStandardType, selectedStandardId, fetchFacultyClasses]);
    ...
    const fetchStandards = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(`✅ [FACULTY ANALYTICS] Loaded standards for term ${selectedTermId}:`, {
          ...
          console.warn('⚠️ [FACULTY ANALYTICS] Failed to fetch standards');
          setAvailableStandards({ so: [], iga: [], cdio: [], sdg: [] });
          setSelectedStandardType('');
          setSelectedStandardId('all');
        }
      } catch (error) {
        console.error('❌ [FACULTY ANALYTICS] Error fetching standards:', error);
        setAvailableStandards({ so: [], iga: [], cdio: [], sdg: [] });
        setSelectedStandardType('');
        setSelectedStandardId('all');
      }
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedCluster, searchQuery, selectedTermId, selectedYearLevel, selectedStandardType, selectedStandardId]);
              ...
              {loadingPhoto && !studentPhoto && !studentPhotoCache.current.has(selectedStudent.student_id) ? (
                ...
                const calculateOverallMetrics = () => {
                  ...
                  if (selectedClassId !== 'all' && classFilteredData) {
                    console.log('📊 [FACULTY ANALYTICS] Using per-class metrics:', {
                  ...
                  console.log('📊 [FACULTY ANALYTICS] Using overall metrics:', {
