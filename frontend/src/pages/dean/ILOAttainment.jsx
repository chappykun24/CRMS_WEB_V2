...
const ILOAttainment = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (activeTerm) {
            ...
            console.log('✅ [DEAN ILO ATTAINMENT] Active term found:', activeTerm.term_id, activeTerm.school_year, activeTerm.semester)
          } else {
            console.warn('⚠️ [DEAN ILO ATTAINMENT] No active term found')
          }
        }
      } catch (error) {
        console.error('❌ [DEAN ILO ATTAINMENT] Error fetching active term:', error)
      }
    }
    fetchActiveTerm()
  }, [])
    ...
    const loadClasses = async () => {
      ...
      try {
        ...
        if (error.name === 'AbortError') {
          console.log('🚫 [DEAN ILO ATTAINMENT] Classes request was aborted')
          return
        }
  ...
  const loadAttainmentData = useCallback(async (sectionCourseId, iloId = null) => {
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        console.log('🚫 [DEAN ILO ATTAINMENT] Attainment request was aborted')
        return
      }
                ...
                {selectedClass && selectedILO && (
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-sm font-semibold text-gray-900">Quick Statistics</h4>
                      <button
                        onClick={() => setShowSummaryModal(true)}
                        className="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors"
