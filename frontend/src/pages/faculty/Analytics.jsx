...
import { getPrefetchedAnalytics, getPrefetchedSchoolTerms, prefetchFacultyData } from '../../services/dataPrefetchService';
...
const Analytics = () => {
    ...
    try {
      ...
      if (specializationsRes.ok) {
        ...
        const filteredSpecializations = Array.isArray(specializationsData) 
          ...
          : [];
        console.log('✅ [FACULTY ANALYTICS] Loaded specializations:', filteredSpecializations.length, filteredSpecializations);
        setSpecializations(filteredSpecializations);
      }
    } catch (error) {
      console.error('❌ [FACULTY ANALYTICS] Error fetching filter options:', error);
    }
  }, []);
  ...
  const handleFetch = useCallback((forceRefresh = false) => {
    ...
    if (selectedStandardType && selectedStandardId && selectedStandardId !== 'all') {
      ...
      console.log(`🎯 [FACULTY ANALYTICS] Adding ${selectedStandardType} filter:`, selectedStandardId);
    }
    ...
    if (forceRefresh) {
      ...
      console.log('🔄 [FACULTY ANALYTICS] Adding force_refresh parameter to API call');
    }
    ...
    fetch(url, {
      ...
      .then((json) => {
        console.log('✅ [FACULTY ANALYTICS] Received data:', json);
        console.log('🎯 [FACULTY ANALYTICS] Clustering enabled:', json.clustering?.enabled);
        ...
        if (json.success) {
          ...
          console.log(`📊 [FACULTY ANALYTICS] Students with clusters: ${studentsWithClusters.length}/${studentsData.length}`);
          ...
          console.log('🔍 [FACULTY ANALYTICS] Clustering enabled status:', json.clustering?.enabled);
          console.log('🔍 [FACULTY ANALYTICS] Backend platform:', json.clustering?.backendPlatform);
          console.log('🔍 [FACULTY ANALYTICS] Clustering API platform:', json.clustering?.apiPlatform);
        } else {
          if (setErrorRef.current) {
            try {
